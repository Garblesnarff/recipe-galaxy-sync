...
import { BookHeart, Settings, ShoppingCart, Utensils, Database } from 'lucide-react';
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t py-2 z-50 shadow-lg">
      <div className="flex justify-around max-w-lg mx-auto">
        <button 
          className={`nav-button ${isActive('/') && !location.pathname.startsWith('/admin') ? 'nav-button-active' : ''}`}
        ...
        <button 
          className={`nav-button ${isActive('/admin') ? 'nav-button-active' : ''}`}
          onClick={() => handleButtonClick(3, '/admin/ingredients')}
          style={{ 
        ...
        <button 
          ...
          onClick={() => handleButtonClick(4, '/settings')}
          style={{ 
            animation: activeRipple === 4 ? 'button-press 0.4s ease' : 'none'
          }}
        >
